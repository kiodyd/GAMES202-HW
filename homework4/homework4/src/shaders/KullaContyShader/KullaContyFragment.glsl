...
vec3 MultiScatterBRDF(float NdotL, float NdotV)
{
  ...
  vec3 one_minus_E_o    = vec3(1.0) - E_o;
  vec3 one_minus_E_i    = vec3(1.0) - E_i;
  vec3 one_minus_E_avg  = vec3(1.0) - E_avg;

  vec3 f_ms  = one_minus_E_o * one_minus_E_i / max(PI * one_minus_E_avg, 0.001);
  vec3 f_add = F_avg * E_avg / max(vec3(1.0) - F_avg * one_minus_E_avg, 0.001);
