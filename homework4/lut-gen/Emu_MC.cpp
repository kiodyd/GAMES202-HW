...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      ...
      Vec3f I = sampleList.directions[i];
      Vec3f H = normalize(V + I);
      float NdotI = dot(N,I);
      float G = GeometrySmith(roughness, NdotV, NdotI);
      float D = DistributionGGX(N, H, roughness);
      float fr = (G * D) / (4 * NdotI * NdotV);
      float E = fr * NdotI / sampleList.PDFs[i];
      A += E; B += E; C += E;
    }
