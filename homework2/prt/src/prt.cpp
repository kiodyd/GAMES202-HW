        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3d _dir = dir.cast<double>().normalized();
                    // 像素足够小，可以当做单位面积
                    float dArea = CalcArea(x, y, width, height);
                    // 遍历球谐基函数
                    for (int l = 0; l <= SHOrder; l++)
                    {
                        for (int m = -l; m <= l; m++)
                        {
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * sh::EvalSH(l, m, _dir) * dArea;
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            // 对给定顶点计算间接球谐系数，使用蒙特卡洛
            auto shInterFunc = [&](const Point3f &point, const Normal3f &normal, int depth){
                std::function<std::vector<double>(const Point3f&, const Normal3f&, int)>subFunc;
                subFunc = [&](const Point3f &point, const Normal3f &normal, int depth){
                    std::vector<double> ret;
                    ret.resize(SHCoeffLength);
                    for (double & i : ret) { i = 0;}
                    ...
                    for (int t = 0; t < sample_side; t++) {
                        for (int p = 0; p < sample_side; p++) {
                            ...
                            const auto wi = Vector3f(d.x(), d.y(), d.z());
                            Intersection in;
                            float V = 1.f - (scene->rayIntersect(Ray3f(point, wi), in) ? 0.f : 1.f);
                            V *= normal.dot(wi);
                            if (V > 0.f)
                            {
                                auto bary = in.bary;
                                auto tri = in.tri_index;
                                auto mesh = in.mesh;

                                auto normals = mesh->getVertexNormals();
                                auto interpNormal = Normal3f(
                                        normals.col(tri[0]) * bary.x() + normals.col(tri[1]) * bary.y() + normals.col(tri[2]) * bary.z()).normalized();

                                auto nextBound = subFunc(in.p, interpNormal, depth + 1);

                                for (int i = 0; i < SHCoeffLength; ++i) {
                                    ret[i] += (m_TransportSHCoeffs.col(tri[0]).coeff(i) * bary.x() +
                                               m_TransportSHCoeffs.col(tri[1]).coeff(i) * bary.y() +
                                               m_TransportSHCoeffs.col(tri[2]).coeff(i) * bary.z() +
                                               nextBound[i]) * weight * V;
                                }
                            }
                        }
                    }
                ...
                return subFunc(point, normal, depth);
            };
            ...
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
                const Point3f &v = mesh->getVertexPositions().col(i);
                const Normal3f &n = mesh->getVertexNormals().col(i);
                auto interSH = shInterFunc(v, n, 1);
                for (int j = 0; j < SHCoeffLength; ++j) {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += interSH[j]/M_PI;
                }
            }
        }
